...
const TodeySales: React.FC<SalesDataProps> = ({ briefinfo }) => {
    ...
    return (
        <div className="bg-white rounded-lg p-4 md:p-6 mb-10" style={{ maxHeight: "350px", boxShadow: "0px 0px 8px 8px #F8F9FA" }}>
            <div>
                <div>
                    <h4 style={{ fontSize: "20px" }}>Өнөөдрийн борлуулалт</h4>
                            ...
                            {partnerHelpdata.map((partner, index) => (
                                <Col className="p-3" key={index} style={{ display: 'inline-block' }}>
                                    <div
                                        ...
                                        <h5 style={{ color: '#151D48', fontSize: '24px' }} className="mt-5">
                                            {partner.name}
                                        </h5>
